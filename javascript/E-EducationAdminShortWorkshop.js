...
document.addEventListener("DOMContentLoaded", function () {
  const filterBtn = document.getElementById("EEASW-filterBtn");
  const sortBtn = document.getElementById("EEASW-sortBtn");

  const filterPopup = document.getElementById("EEASW-filterPopup");
  const sortPopup = document.getElementById("EEASW-sortPopup");

  const table = document.querySelector(".EEASW-table");
  const countText = document.querySelector(".EEASW-count");

  let rows = Array.from(
    document.querySelectorAll(".EEASW-table .EEASW-row:not(.EEASW-header-row)"),
  );
  ...
  function checkNoRows() {
    ...
    let noRow = document.getElementById("EEASW-noRow");

    if (visibleRows.length === 0) {
      if (!noRow) {
        noRow = document.createElement("div");
        noRow.id = "EEASW-noRow";
        noRow.className = "EEASW-row";
  ...
  const checkboxes = document.querySelectorAll(
    "#EEASW-filterPopup input[type='checkbox']",
  );
  ...
  checkboxes.forEach((box) => {
    box.addEventListener("change", function () {
      ...
      rows.forEach((row) => {
        const status = row
          .querySelector(".EEASW-status-col span")
          .innerText.trim();
  ...
  const sortOptions = document.querySelectorAll("#EEASW-sortPopup p");
...
function updateMobCount() {
  const mobCount = document.querySelector(".EEASW-mob-count");
  if (!mobCount) return;
  const mobItems = document.querySelectorAll(".EEASW-acc-item");
  mobCount.innerText = `${mobItems.length} Ongoing`;
}

updateMobCount();

const accHeaders = document.querySelectorAll(".EEASW-acc-header");

accHeaders.forEach((header) => {
  header.addEventListener("click", () => {
    const item = header.parentElement;
    const body = header.nextElementSibling;
    const arrow = header.querySelector(".EEASW-arrow");
...
document.querySelector("#account-menu .mobile-dropdown:nth-child(7) li:nth-child(2)").classList.add("submenu-active-page");
