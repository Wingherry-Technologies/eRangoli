...
document.addEventListener("DOMContentLoaded", function () {
  const filterBtn = document.getElementById("EEDSWT-filterBtn");
  const sortBtn = document.getElementById("EEDSWT-sortBtn");

  const filterPopup = document.getElementById("EEDSWT-filterPopup");
  const sortPopup = document.getElementById("EEDSWT-sortPopup");

  const table = document.querySelector(".EEDSWT-table");
  const countText = document.querySelector(".EEDSWT-count");

  let rows = Array.from(
    document.querySelectorAll(
      ".EEDSWT-table .EEDSWT-row:not(.EEDSWT-header-row)",
    ),
  );
  ...
  function checkNoRows() {
    ...
    let noRow = document.getElementById("EEDSWT-noRow");

    if (visibleRows.length === 0) {
      if (!noRow) {
        noRow = document.createElement("div");
        noRow.id = "EEDSWT-noRow";
        noRow.className = "EEDSWT-row";
  ...
  const checkboxes = document.querySelectorAll(
    "#EEDSWT-filterPopup input[type='checkbox']",
  );
  ...
  checkboxes.forEach((box) => {
    box.addEventListener("change", function () {
      ...
      rows.forEach((row) => {
        const status = row
          .querySelector(".EEDSWT-status-col span")
          .innerText.trim();
  ...
  const sortOptions = document.querySelectorAll("#EEDSWT-sortPopup p");
...
function updateMobCount() {
  const mobCount = document.querySelector(".EEDSWT-mob-count");
  if (!mobCount) return;
  const mobItems = document.querySelectorAll(".EEDSWT-acc-item");
  mobCount.innerText = `${mobItems.length} Ongoing`;
}

updateMobCount();

const accHeaders = document.querySelectorAll(".EEDSWT-acc-header");

accHeaders.forEach((header) => {
  header.addEventListener("click", () => {
    const item = header.parentElement;
    const body = header.nextElementSibling;
    const arrow = header.querySelector(".EEDSWT-arrow");
...
document.querySelector(".sidebar-main-vendor ul>ul:nth-of-type(3)>li:nth-child(3)").classList.add("submenu-active-highlight");
...
document.querySelector("#account-menu .mobile-dropdown:nth-child(5) li:nth-child(3)").classList.add("submenu-active-page");
