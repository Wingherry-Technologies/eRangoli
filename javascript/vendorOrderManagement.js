...
document.querySelectorAll(".open-modal-popup-mobile").forEach((btn) => {
  btn.addEventListener("click", function () {
    ...
    const status = faqAnswer.querySelector(
      ".received-orders, .packed-order, .delivered-order",
    );
    if (!status) return;
...
function sortContent() {
  ...
  else if (document.getElementById("quantity-lessthan-10").checked)
    threshold = 10;
  else if (document.getElementById("quantity-lessthan-20").checked)
    threshold = 20;
  ...
  const sortedFaqs = faqArray
    .filter((item) => {
      const qty = parseInt(
        item.querySelector(".all-answers div:nth-child(4) span:last-child")
          .innerText,
      );
      return greaterThan ? qty > threshold : qty < threshold;
    })
    .sort((a, b) => {
      const qtyA = parseInt(
        a.querySelector(".all-answers div:nth-child(4) span:last-child")
          .innerText,
      );
      const qtyB = parseInt(
        b.querySelector(".all-answers div:nth-child(4) span:last-child")
          .innerText,
      );
      return qtyA - qtyB;
    });
...
document
  .querySelector("#sidebar-main-vendor ul>li:nth-child(3)")
  .classList.add("sidebar-active");

document
  .querySelector("#account-menu li:nth-child(3) .dropdown-header")
